...
GD_code = f'{working_dir}/scripts/docking_new.py'

initial_start = sys.argv[2]
res_file = sys.argv[3]
out_dir = sys.argv[4]

weight_file = f'{working_dir}/talaris2013.wts'
num_of_jobs = 5
thre = 0.0001
...
GD_pdb = f'{out_dir}/{target_name}_predicted.pdb'



predicted_pose = pyrosetta.pose_from_pdb(f'{out_dir}/{target_name}_predicted.pdb')
restraint_file = res_file
...
class env():
    ...
    def step(self, action):
        ...
        if curr_fnat > GD_fnat:
          ...
          reward  = 100
          self.pose.dump_pdb(f'{out_dir}/{target_name}_rl.pdb')
          exit()
...
Dict[0] = env(GD_pdb, GD_pdb, target_name)
...
for i in trange(10**7):
    ...
    train()
    saver.save(sess, f'./my_model_{target_name}')
