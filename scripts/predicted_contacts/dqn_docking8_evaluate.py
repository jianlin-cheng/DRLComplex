...
#targets_file = '/storage/hpc/data/esdft/std32/targets_list.txt'
#initial_pdbs = '/storage/hpc/data/esdft/initial_start_std32/'
#true_pdbs = '/storage/hpc/data/esdft/std32/pdbs/'


predicted_pose = pyrosetta.pose_from_pdb('./1B70A_1B70B_GD.pdb')
restraint_file = './1B70A_1B70B.rr'
true_contacts = read_res(predicted_pose, restraint_file)

fnat_GD = fnat(predicted_pose, true_contacts)
...
class env():
    ...
    def __init__(self, true_pdb_file, pdb_file, target_name):
        ...
        self.target_name = ntpath.basename(true_pdb_file).split('.')[0]


        self.prev_ca_rmsd = CA_rmsd(self.true_pose, self.pose)
        self.prev_fnat = fnat(self.pose, true_contacts)
        self.n_actions = 12


        self.all_residues_ids_chain1 = pyrosetta.rosetta.core.pose.get_resnums_for_chain(self.pose, self.target_name.split('_')[0][-1])
        self.all_residues_ids_chain2 = pyrosetta.rosetta.core.pose.get_resnums_for_chain(self.pose, self.target_name.split('_')[1][-1])
    ...
    def step(self, action):
        ...
        curr_fnat = fnat(self.pose, true_contacts)
        ...
        #if curr_ca_rmsd <= 0.1:
        if curr_fnat > fnat_GD:
          done = True
          reward  = 100
          env1.pose.dump_pdb('1B70A_1B70B_rl.pdb')
          ...
          diff1 = self.prev_ca_rmsd - curr_ca_rmsd
          diff = curr_fnat - self.prev_fnat
          reward = diff
...
class ExperienceBuffer:
    def __init__(self, states, actions, rewards, done_flags, next_states):
        self.states = states
    ...
    def serialize(self, h5file):
        ...
        h5file['experience'].create_dataset('done_flags', data=self.done_flags)
        h5file['experience'].create_dataset('next_states', data=self.next_states)
    ...
    def load_experience(h5file):
        return ExperienceBuffer(
            ...
            done_flags=np.array(h5file['experience']['done_flags']),
            next_states=np.array(h5file['experience']['next_states']))
...
#Dict[0] = env(f'{true_pdbs}/{targets[index]}.pdb', f'{initial_pdbs}/{targets[index]}.start.pdb')

Dict[0] = env('./1B70A_1B70B_GD.pdb', './1B70A_1B70B.start.pdb', '1B70A_1B70B')
...
def evaluate(env, agent, n_games=1, greedy=False, t_max=10000):
    ...
    with open(f'{os.getcwd()}/rewards_{index}.txt', 'a') as f:
      f.write(str(mean_reward))
      f.write('\t')
      f.write(str(env.prev_ca_rmsd))
      f.write('\t')
      f.write(str(env.prev_fnat))
      #f.write('\t')
...
while True:
    new_saver = tf.train.import_meta_graph(f'./my_model_{index}.meta')
    new_saver.restore(sess, tf.train.latest_checkpoint('./'))

    #for t in range(num_targets):
    evaluate(Dict[0], agent, n_games=1)

#evaluate(env1, agent, n_games=1)
#evaluate(env2, agent, n_games=1)
#evaluate(env3, agent, n_games=1)
#evaluate(env4, agent, n_games=1)
#evaluate(env5, agent, n_games=1)
#evaluate(env6, agent, n_games=1)
#evaluate(env7, agent, n_games=1)

'''def play_and_record(env):
    ...
    buffer = ExperienceBuffer(np.array(states), np.array(actions), np.array(rewards), np.array(done_flags), np.array(next_states))
    
    with h5py.File(f'{env.target_name}/{env.target_name}_{time.time()}.hdf5', 'w') as experience_outf:
        buffer.serialize(experience_outf)
 

if not os.path.exists(f'{env1.target_name}'):
    os.system(f'mkdir {env1.target_name}')
while True:
    play_and_record(env1)'''
