...
GD_code = '/bml/elham/1B70A_1B70B_cnt/scripts/docking_new.py'
initial_start = '/bml/elham/1B70A_1B70B_cnt/1B70A_1B70B.start.pdb'
res_file = '/bml/elham/1B70A_1B70B_cnt/1B70A_1B70B.rr'
out_dir = '/bml/elham/1B70A_1B70B_cnt/output_dir/'
weight_file = '/bml/elham/1B70A_1B70B_cnt/talaris2013.wts'
num_of_jobs = 5
thre = 0.0001
target_name = '1B70A_1B70B'


cmd = f'python {GD_code} {initial_start} {res_file} {out_dir} {weight_file} {num_of_jobs} {thre} {target_name}'
os.system(cmd)
...
GD_fnat = fnat(predicted_pose, true_contacts)
...
class env():
    ...
    def __init__(self, true_pdb_file, pdb_file, target_name):
        ...
        self.all_residues_ids_chain1 = pyrosetta.rosetta.core.pose.get_resnums_for_chain(self.pose, target_name.split('_')[0][-1])
        self.all_residues_ids_chain2 = pyrosetta.rosetta.core.pose.get_resnums_for_chain(self.pose, target_name.split('_')[1][-1])
...
#print(targets[j])
#Dict[0] = env(f'{true_pdbs}/{targets[index]}.pdb', f'{initial_pdbs}/{targets[index]}.start.pdb', targets[index])
...
#env1.reset()
#env1.pose.dump_pdb('1B70A_1B70B.start.pdb')
...
#print(targets[t])
play_and_record(agent, Dict[0], exp_replay1, n_steps=10000)
...
for i in trange(10**7):
    ...
    #s = env.restore_pose()
    #for _ in range(1):
    play_and_record(agent, Dict[0], exp_replay1, 10)
    #env.set_prev_pose()
    ...
    train()
    saver.save(sess, f'./my_model_{index}')
