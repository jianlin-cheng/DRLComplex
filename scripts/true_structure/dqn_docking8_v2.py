...
class env():
    ...
    def get_distance_map(self):
        ...
        xyz_CA_chain1 = np.array([env.pose.residue(r).xyz('CA') for r in self.all_residues_ids_chain1])
        xyz_CA_chain2 = np.array([env.pose.residue(r).xyz('CA') for r in self.all_residues_ids_chain2])
...
class DQNAgent:
    def __init__(self, name, state_shape, n_actions, epsilon=0, reuse=False):
        ...
        with tf.variable_scope(name, reuse=reuse):
            ...
            self.network.add(L.Conv2D(32, (8, 8), strides=[4,4,], activation='relu', input_shape=state_shape))
            self.network.add(L.Conv2D(64, (4, 4), strides=[2,2,], activation='relu'))
            self.network.add(L.Conv2D(64, (3, 3), strides=[1,1,], activation='relu'))
...
env = env('/home/esdft/DeepRLP/data/DOE_experiment/true_pdbs_100/1A19A_1A19B.pdb', '/home/esdft/DeepRLP/data/DOE_experiment/initial_start_100/1A19A_1A19B.start.pdb')
...
def evaluate(env, agent, n_games=1, greedy=False, t_max=10000):
    ...
    with open('/home/esdft/DeepRLP/rewards_1A19.txt', 'a') as f:
