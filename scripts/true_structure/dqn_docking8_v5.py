...
env1 = env('/home/esdft/DeepRLP/data/3HE4A_3HE4B.pdb', '/home/esdft/DeepRLP/data/3HE4_AB.pdb')
env2 = env('/home/esdft/DeepRLP/data/5C39.pdb', '/home/esdft/DeepRLP/data/5C39_AB.pdb')
...
'''def evaluate(env, agent, n_games=1, greedy=False, t_max=10000):
    ...
    with open('/home/esdft/Desktop/rewards.txt', 'a') as f:
...
def play_and_record(env):
    ...
    #return s, a, r, done, next_s



envs = [env1, env2, env3, env4, env5, env6, env7]
...
while True:
    for counter in range(1000):
        ...
        for e in envs:
            play_and_record(e)
            #s, a, r, done, next_s = play_and_record(e)
            #states.append(s)
            #actions.append(a)
            #rewards.append(r)
            #done_flags.append(done)
        ...
        obs_batch, act_batch, reward_batch, next_obs_batch, is_done_batch = sklearn.utils.shuffle(np.array(states), np.array(actions), np.array(rewards), np.array(done_flags), np.array(next_states))
        buffer = ExperienceBuffer(obs_batch, act_batch, reward_batch, is_done_batch, next_obs_batch)
    
        with h5py.File(f'generated_training_data/data_{counter}.hdf5', 'w') as experience_outf:
