...
#states, actions, rewards, done_flags, next_states = load_experience(h5py.File('generated_training_data/data_0.hdf5'))
...
env1 = env('/home/esdft/DeepRLP/data/3HE4A_3HE4B.pdb', '/home/esdft/DeepRLP/data/3HE4A_3HE4B.start.pdb')
...
def get_data_from_hdf5(hdf5_path):
    with h5py.File(hdf5_path, 'r') as f:
        
        for key in f.keys():
            group = f[key]
            actions = np.array(group['actions'])
            done_flags = np.array(group['done_flags'])
            next_states = np.array(group['next_states'])
            rewards = np.array(group['rewards'])
            states = np.array(group['states'])

    return actions, done_flags, next_states, rewards, states




actions, done_flags, next_states, rewards, states = get_data_from_hdf5("generated_training_data/data_1.hdf5")


start = time.time()

_, loss_t = sess.run([train_step, td_loss], {obs_ph:states[0:256], actions_ph:actions[0:256], rewards_ph:rewards[0:256], next_obs_ph:next_states[0:256], is_done_ph:done_flags[0:256]})
