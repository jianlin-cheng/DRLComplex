...
class env():
    ...
    def __init__(self, true_pdb_file, pdb_file, target_name):
        ...
        self.original_CA_chain2 = get_ca_from_chain(chain2) #this should by true array initial
        self.original_atoms_chain2 = get_atoms_from_chain(chain2)


        self.true_ca_atoms = get_CA_atoms(target_name, true_pdb_file)  #true_pdb_file is thre true array
...
for i in trange(10**5):
    ...
    new_saver = tf.train.import_meta_graph('my_model.meta')
    new_saver.restore(sess, tf.train.latest_checkpoint('./')) #this is restoring the model and start evaluating
    ...
    '''for t in range(num_targets):
        play_and_record(agent, Dict[t], exp_replay1, 1)
    #env.set_prev_pose()
