...
for j in range(num_targets):
    Dict[j] = env(f'{true_pdbs}/{targets[j]}.pdb', f'{initial_pdbs}/{targets[j]}.start.pdb')
...
for t in range(num_targets):
    evaluate(Dict[t], agent, n_games=1)'''
...
def play_and_record(env, agent):
    ...
    return states, actions, rewards, done_flags, next_states
...
for itr in range(10**9):
    ...
    for j in range(num_targets):
        s, a, r, done, next_s = play_and_record(Dict[j], agent)
        states.extend(s)
        actions.extend(a)
        rewards.extend(r)
        done_flags.extend(done)
        next_states.extend(next_s)
    

    obs_batch, act_batch, reward_batch, is_done_batch, next_obs_batch = sklearn.utils.shuffle(np.array(states), np.array(actions), np.array(rewards), np.array(done_flags), np.array(next_states))
    buffer = ExperienceBuffer(obs_batch, act_batch, reward_batch, is_done_batch, next_obs_batch)
    
    with h5py.File(f'generated_training_data/data_{number}.hdf5', 'w') as experience_outf:
